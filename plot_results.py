"""
Generate evaluation plots. Run from project root:
...
def plot_results(dataset):
    ...
    # Figure 1: Performance decay
    plt.figure(figsize=(9, 5))
    plt.plot(X, mean_evals["PAK"], marker="o", label="Precision@K")
    plt.plot(X, mean_evals["RR"], marker="o", label="Reciprocal Rank")
    plt.plot(X, mean_evals["AP"], marker="o", label="Average Precision")
    plt.xticks(X, conds)
    ...
    plt.figure(figsize=(9, 5))
    plt.errorbar(X, mean_ap, yerr=sem_ap, fmt="o-", linewidth=3, capsize=6,
                 label="Mean AP ± SEM", zorder=5)
    plt.xticks(X, conds)
...
if __name__ == "__main__":
    if len(sys.argv) < 2:
        print("Usage: python plot_results.py <cranfield|lisa>")
        sys.exit(1)
